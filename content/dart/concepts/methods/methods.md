...
A class called Hello contains a method called 'helloWorld' that prints "Hello World!":
...
The method 'subtraction' finds the difference of two numbers and returns the calculation as an *int* value:
...
void Main() {
    ...
    bool boolean = data.returnAnyType(4);              // returns a boolean value
    print(booleanValue);
...
from outside its defined scope and prints a statement using that argument:
...
parameters respectively. If the invoked method call does not have the exact number of required arguments it needs or if 
...
class Hello {
    ...
    // The String parameter 'lastName' is set to a default empty string ("")
    static void greetings(String firstName, [String lastName = ""]) {
...
```
In order to pass a value to a named parameter, the value must be referenced by name in the method call. 
For example, the 'measure' method uses two named parameters, {int? width} and {int? height}, along with 
a required parameter (int length), and calculates a measurement:

```dart
void main() {
    Math.measure(5);                        // No refenences for named parameters
    Math.measure(5, width: 3);              // 'width: 3' references the 'width' paramter by name
    Math.measure(5, height: 6, width: 3);   // 'height: 6' and 'width: 3' reference 'width' and 'height' parameters by name
}

class Math {

    // This method contains named parameters: {int? width}, {int? height}
...
An argument that is referenced by name in a method call doesn't have to be in the same position like its 
named parameter is in a method declaration, only as long as they are not mixed up with positioned parameters:
...
// Incorrect: Position of 'width: 3' overlaps with positioned parameter 'int length'
Math.measure(width: 3, height: 6, 5);
```
...
the specified value by default.

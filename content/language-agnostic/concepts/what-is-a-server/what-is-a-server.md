...
The internet largely runs based on this "Client-Server" framework. The term server is broad enough to encompass the myriad of specific servers that can be found today such as web servers, game servers, email servers and proxy servers to name a few.

Large groups of servers can be referred to as data centers. At data center level scale, maintenance of servers requires diligent control of physical environments such as the temperature of locations housing servers, redundancy etc. Cooling servers is very important at data center level server scale because servers emit heat (especially in close proximity to one another) and therefore need to be cooled down with fans and temperature control measures like air conditioning which at scale can be quite substantial in terms of resources. 

...
A heuristic function, **h(x)**, evaluates the successive node based on how close it is to the goal node(as determined by the heuristic value). In other words, it chooses the immediate low-cost option. As this is the case, however, it does not necessarily find the shortest path to the goal.

...
The **`.linspace()`** function returns an array of evenly-spaced numbers over a given interval. It returns a specified number of values evenly spaced over the specified interval \[`start`,`stop`\], optionally excluding the `stop` value.

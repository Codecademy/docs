...
Unlike the built-in [`.transpose()`](https://www.codecademy.com/resources/docs/numpy/built-in-functions/reshape) function, the `.transpose()` method can accept the axes as seprate arguments, as well as in the form of a tuple: i.e. `array.transpose(2,3,1)` is the same as `array.transpose((2,3,1))`. 

...
The bigger the table, the more important indices are. Indices are the gateway to the data that is stored in a table. The following visual may help. You have a group of photos. When the number of photos is small, they are easy to find. Who needs organization?  When the number of photos and a particular one is hard to find, then you may need to organize them.  If they are physical photos, you organize them in boxes and label the boxes.  Those labels are indices.  These labels allow you to get to the photo quicker.  For those smart phone users, the photos are put in virtual folders and some software allows you to categorize the photos with labels.     
...
##**Specific answers:**
1. size is relative.  What is large today will be small tomorrow but I would consider any table over 10,000 rows as large.  
...
##**Complexity:** 
When a index exists for a table, the database must decide whether to use that index to make access faster, or just scan the table. This decision must be made quickly. The algorithms used are rocket science.  This is what separates a great database from one that is not.  To add to the complexity, a databsase may use an index if it is close but not perfect fit.  Finally, it may use multiple indices if that gives a close fit.  The point is that the process is complex, which is why you need use the analysis tools.  These give you a view into the mind of the database (aka optimizer).

##**Real life story:** 

I worked for a major retailer and one of the end of day processes was taking about 3 and half hours to complete.  The code was reviewed an blocking records was tried.  This made no difference. I had read somewhere that indices could cuase slow performance on large files where large amounts of data was added. The file being writted to had over 30 indices.  A coworker and I decided to see what indices could be removed without impacted other applications.  We consulted with numerous people and concluded only 9 of the existing indices were needed.  We went ahead and removed all but the 9.  The job was  monitored and it went from 3 hours 45 minutes to 45 minutes. 

Moral of the story:  Add indices when needed but do with care.  Also monitor the install.    -  

##**Best practices to consider**
1. Use Analyze and visual tools to research how a database optimizes queries for a table 
2. Teamwork.  You will need to consult others.  If there is a database administrator, they may be a good source to conslut with.  
3. monitor installs.  The decisions of the optimizer change with a growth or other changes to a table.  

...
`XMLHttpRequest`(XHR) is a built-in browser object that allows HTTP requests written in JavaScript; the modern alternative method is `fetch()`.

## Getting Started

`XMLHttpRequest` can be used in two modes: synchronous and asynchronous.

**Note**: Synchronous XHR will be deprecated because the browser runs on a main thread. This thread handles all the events that occur on the page. Synchronous XHR impedes on the browsers ability to process these events and can disrupt a user's experience.
...
**Note**: The `open` call, contrary to its name, does not open the connection. It only configures the request. The network activity starts with the call of `send`.
...
These three events are most widely used:
...
If the request does not succeed within the given time, it gets canceled and the `timeout` event triggers.

### Synchronous Method

If in the `.open` method, the third parameter `async` is set to `false`, the request is made synchronously.

It might look good, but synchronous calls are rarely used because they block in-page JavaScript until loading is complete. In some browsers, it becomes impossible to scroll. If a synchronous call takes too much time, the browser may suggest to close the “hanging” webpage.

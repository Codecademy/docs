...
In this example, we create a new `Date` object representing the current date and time. We then use `setUTCMilliseconds()` to modify the milliseconds part of the date to 500. The other date and time components (year, month, day, hour, minute, second) remain unchanged
